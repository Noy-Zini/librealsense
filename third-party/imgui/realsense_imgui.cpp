...
bool ImGui::VSliderFloat(const char* label, const ImVec2& size, float* v, float v_min, float v_max, const char* display_format, float power, bool render_bg)
{
    ...
    const ImRect bb(frame_bb.Min,ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x+ frame_bb.Max.x : 0.0f+frame_bb.Max.x, 0.0f+ frame_bb.Max.y));

    ItemSize(bb, style.FramePadding.y);
    if (!ItemAdd(frame_bb, id))
        return false;

    const bool hovered = IsHovered(frame_bb, id,false);
    if (hovered)
        SetHoveredID(id);
    ...
    RenderTextClipped(ImVec2(frame_bb.Min.x, frame_bb.Min.y + style.FramePadding.y), frame_bb.Max, value_buf, value_buf_end, NULL, style.WindowTitleAlign);
